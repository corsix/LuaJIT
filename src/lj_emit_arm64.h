...
{
  /* See https://dougallj.wordpress.com/2021/10/30/ */
  int rotation, ones, size, immr, imms;
  if (!is64) n = ((uint64_t)n << 32) | (uint32_t)n;
  if ((n+1u) <= 1u) return 0;  /* Neither all-zero nor all-ones are allowed. */
  rotation = n & (n+1u) ? emit_ctz64(n & (n+1u)) : 64;
  n = lj_ror(n, rotation & 63);
  ones = emit_ctz64(~n);
  ...
  while (work) {
    ...
    if (ref < REF_TRUE) {
      uint64_t kx = ra_iskref(ref) ? (uint64_t)ra_krefk(as, ref) :
				     get_k64val(as, ref);
      int64_t delta = is64 ? (int64_t)(k - kx) :
			     (int32_t)((uint32_t)k - (uint32_t)kx);
      if (delta == 0) {
...
static void emit_lsptr(ASMState *as, A64Ins ai, Reg r, void *p)
{
  ...
  if (emit_checkofs(ai, ofs)) {
    ...
    /* IP + offset is cheaper than allock, but requires to be in range. */
    emit_d(as, A64I_LDRLx | A64F_S19(mcpofs(as, p)>>2), r);
    return;
  } else {
    /* Split up into base reg + offset. */
...
static void emit_call(ASMState *as, ASMFunction target)
{
  ...
  if (A64F_S_OK(delta>>2, 26)) {
    *p = A64I_BL | A64F_S26(delta>>2);
	...
	RSET_SCRATCH_GPR & ~RSET_RANGE(REGARG_FIRSTGPR, REGARG_LASTGPR+1));
